...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const policies: { agency_id: bigint | null; agencies?: { name: string | null } | null }[] = await prisma.policies.findMany({
      where: agency_id
    ...
    const agencyDetails = policies
      .filter((policy: { agency_id: bigint | null; agencies?: { name: string | null } | null }) => policy.agency_id !== null) // Filter keluar nilai null
      .map(policy => ({
